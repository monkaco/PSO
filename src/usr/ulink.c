...
void uart_write2 (uint16_t* uart_tx_buf, ulink_pso_data_t* dp)
{
	...
	uart_tx_buf[2] = 0x00FF & (index >> 8);
	uart_tx_buf[3] = 0x00FF & (index);
	...
	aux = create_checksum(uart_tx_buf, 17);

	uart_tx_buf[18] = 0x00FF & (aux >> 8);
	uart_tx_buf[19] = 0x00FF & (aux);
