 ...
 * board.
 *
 * MMC/SD  : EK-TM4C123GXL (Tiva-c)
 * ----------------------------------------------------------------------------
 * SCK     : connected to PA2 (SSI0Clk)
 * MISO    : connected to PA4 (SSI0Rx)
 * MOSI    : connected to PA5 (SSI0Tx)
 * CARD_CS : connected to PB2 GPIO output
 * ------- : connected to PA3 (SSI0Fss) <- Not used
...
/* Prototypes */
void make_pa5_gpio(void);
void make_pa5_ssi0tx(void);
void sd_ok(void);
void sd_error(void);
...
//#define SDC_SSI_PINS            (SDC_SSI_TX | SDC_SSI_RX | SDC_SSI_CLK |      \
                                 SDC_SSI_FSS)
void make_pa5_gpio(void){
	GPIO_PORTA_DIR_R |=  GPIO_PIN_5;
	GPIO_PORTA_AFSEL_R &= ~GPIO_PIN_5;           /* Disable alt funct on PA5 */
	GPIO_PORTA_PCTL_R &= ~GPIO_PCTL_PA5_M;             /* GPIO Mux Selection */
	GPIO_PORTA_DATA_R |= GPIO_PIN_5;                             /* PA5 high */
}
void make_pa5_ssi0tx(void){
    GPIO_PORTA_AFSEL_R |= GPIO_PIN_5;             /* Enable alt funct on PA5 */
    GPIO_PORTA_PCTL_R  |= GPIO_PCTL_PA5_SSI0TX;            /* SSI0TX on PA5  */
}

// asserts the CS pin (PB2) to the card
static
void SELECT (void)
{
	GPIO_PORTB_DATA_R &= ~(GPIO_PIN_2);  /* CS on PB2 */
	//GPIO_PORTA_DATA_BITS_R &= ~(GPIO_PIN_3);  /* CS on PA3 */
}

// de-asserts the CS (PB2) pin to the card
static
void DESELECT (void)
{
    GPIO_PORTB_DATA_R |= GPIO_PIN_2;  /* CS on PB2 */
...
* Output         : None
* Return         : None
* Description    : Indicates it's OK with the SD card.
*******************************************************************************/
void sd_ok(void)
{
	/* Turn on green LED for 400ms (4*100ms) */
	Timer1 = 4;
	do
	{
	...
	/* Turn off red LED for 200ms (2*100ms) */
	Timer2 = 2;
	do
	{
	    GPIO_PORTF_DATA_R &= ~GPIO_PIN_3;    /* Turn off red LED */
	}while (Timer2);
}


/*******************************************************************************
* Function Name  : sd_error
* Input          : None
* Output         : None
* Return         : None
* Description    : Indicates a problem with the SD card.
*******************************************************************************/
void sd_error(void)
{
	/* Turn on red LED for 500ms (5*100ms) */
	Timer1 = 3;
	do
	{
	    GPIO_PORTF_DATA_R |= GPIO_PIN_1;    /* Turn on red LED */
	}while (Timer1);

	/* Turn off red LED for 500ms (5*100ms) */
	Timer2 = 3;
	do
	{
	    GPIO_PORTF_DATA_R &= ~GPIO_PIN_1;    /* Turn off red LED */
	}while (Timer2);
}

/*******************************************************************************
* Function Name  : sd_stand_by
* Input          : None
* Output         : None
* Return         : None
* Description    : Indicates the Stand-by state.
*******************************************************************************/
void sd_stand_by(void)
{
	/* Turn on yellow (red + green) LED for 300ms (3*100ms) */
	Timer1 = 3;
	do
	{
	    GPIO_PORTF_DATA_R |= (GPIO_PIN_1 | GPIO_PIN_3);   /* Turn on red LED */
	}while (Timer1);

	/* Turn off yellow (red + green) LED for 300ms (3*100ms) */
	Timer2 = 3;
	do
	{
	    GPIO_PORTF_DATA_R &= ~(GPIO_PIN_1 | GPIO_PIN_3); /* Turn off red LED */
	}while (Timer2);
}

/*******************************************************************************
* Function Name  : sd_finish_record
* Input          : None
* Output         : None
* Return         : None
* Description    : Indicates the end of the recording process to the SD card.
*******************************************************************************/
void sd_finish_record(void)
{
	/* Turn on white (all) LEDS for 100ms (1*100ms) */
	Timer1 = 1;
	do
	{
	...
	/* Turn off white (all) LEDS for 100ms (1*100ms) */
	Timer2 = 1;
	do
	{
	    GPIO_PORTF_DATA_R &= ~(GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3);
...
void xmit_spi(BYTE dat)
{
    ...
    while ((SSI0_SR_R & SSI_SR_TNF) == 0U){};     /* wait until room in FIFO */
    SSI0_DR_R = dat;                        /* Write the data to the tx fifo */

    while((SSI0_SR_R & SSI_SR_RNE) == 0U){};    // wait until response
    byte_rx = SSI0_DR_R;
}
...
BYTE rcvr_spi (void)
{
    ...
    while ((SSI0_SR_R & SSI_SR_TNF) == 0U){};     /* Wait until room in FIFO */
    SSI0_DR_R = 0xFF;                                    /* Write dummy data */
...
void send_initial_clock_train(void)
{
    ...
    for(i = 0 ; i < 10 ; i++)
    {
        ...
        /* Write DUMMY data */
        while ((SSI0_SR_R & SSI_SR_TNF) == 0U){}; /* wait until room in FIFO */
        SSI0_DR_R = 0xFF;                     /* Write the data to the tx fifo */
...
void power_on (void)
{
     ...
     *
     * Actually, this initialization was done in "pso_init.c" file.
     */
...
//                           SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, i, 8);
     ...
     *************************************************************************/
    /* CPSDVSR = 2 (0x0A=10)    */
    SSI0_CPSR_R = (SSI0_CPSR_R & ~SSI_CPSR_CPSDVSR_M) + 0x00000002;
...
DWORD get_fattime (void)
{
    ...
    return    ((2015UL-1980) << 25)    // Year = 2007
            | (12UL << 21)             // Month = June
            | (5UL << 16)              // Day = 5
            | (18U << 11)              // Hour = 11
            | (31U << 5)               // Min = 38
